...
🔗 **Backend Repo:** [skillbridge-backend](https://github.com/Rezwan66/skillbridge-backend/tree/dev-a5)
...
- **Rich Tutor Detail Pages** — Full tutor profile with cover image, skills/tags, availability schedule, reviews, and a "Related Tutors" recommendation section.
...
- **Analytics Charts** — Visual dashboard statistics using Recharts AreaCharts and BarCharts, populated from live `/stats` backend endpoints.
...
- **Demo Login Buttons** — One-click credential fill for Student, Tutor, and Admin demo accounts on the login page.
- **Dynamic Subjects Navbar** — The "Subjects" dropdown in the navbar is populated dynamically from the categories API at render time, linking directly to filtered tutor listings.
- **Floating AI Chatbot (Bridget)** — A premium floating chatbot widget visible on all public and dashboard pages, ready for OpenRouter AI integration.
- **Dark / Light Mode** — Full one-click theme toggle powered by `next-themes` and the `ModeToggle` component.
- **Responsive Design** — Fully responsive layout with a collapsible sidebar, mobile sheet navigation, and adaptive grid layouts.
- **Micro-Animations** — Premium hover effects on tutor cards (lift + image zoom), smooth transitions, and interactive filter states powered by Framer Motion.
...
- **About & Contact Pages** — Fully designed pages with core values, team section, contact form, and support information.
...
│   │   ├── page.tsx            # Homepage (8-section landing page)
│   │   ├── about/              # About Us page
...
├── components/
│   ├── layout/                 # App sidebar, navbar, footer, mode toggle, FloatingChat
│   ├── modules/                # Feature-specific components
│   │   ├── authentication/     # Login/Register forms (with demo login buttons)
│   │   ├── dashboard/          # Dashboard components (per-role, with charts)
│   │   ├── home/               # Homepage sections (hero, stats, categories, tutors, etc.)
│   │   ├── shared/             # Shared components (profile card, logo)
│   │   └── tutors/             # Tutor cards, filters, grid, detail page
...
- **Ready for OpenRouter AI integration** — replace the `getMockResponse()` function with a live API call
...
5. Protected route redirects preserve the intended destination through `?redirectPath=` query parameters — even across the Register → Login chain.
